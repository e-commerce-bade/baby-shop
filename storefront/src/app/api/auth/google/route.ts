...
export async function POST(request: Request) {
  ...
  try {
    response = await fetch(buildBackendUrl('/api/v1/auth/google'), {
      method: 'POST',
      headers: {
  ...
  if (!response.ok) {
    return NextResponse.json(payload ?? { message: 'Google ile giriş başarısız.' }, {
  ...
  if (!token) {
    return NextResponse.json({ message: 'Giriş yanıtı bir jeton içermiyor.' }, { status: 502 })
  }
